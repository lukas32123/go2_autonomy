...
parser.add_argument("--szene", type=str, default="ringflur",
                    choices=["ringflur", "saeulenraum"],
                    help="Testumgebung: ringflur (Uni-Ringflur) oder saeulenraum.")
...
def design_scene_ringflur() -> Articulation:
...
def design_scene_saeulenraum() -> Articulation:
    import math
...
def design_scene() -> Articulation:
    """Waehlt die Testumgebung anhand des Arguments --szene aus.

    Beide Umgebungen bleiben unveraendert in je einer eigenen Funktion. Diese
    Weiche trifft nur die Auswahl, damit ein Klon ueber einen einzigen Parameter
    umschaltet und die Geometrie bitgenau erhalten bleibt.
    """
    if args_cli.szene == "saeulenraum":
        return design_scene_saeulenraum()
    return design_scene_ringflur()
